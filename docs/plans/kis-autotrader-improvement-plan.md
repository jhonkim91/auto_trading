...
4. [x] 4차 운영 안정성: 일일 손실/MDD 상태 영속화
5. [x] 5차 문서 대비 보강: 설정 재귀 병합, 국내/미국 스크리너, 종목별 처리 타임아웃, 텔레그램 중지/표시 문구 정리
6. [ ] 6차 유지보수: `autotrader.py`와 `src/` 중복 축소 및 텔레그램 `/daily` 리포트 확장
...
## 4차 구현 결과
- `src/risk.py`와 `autotrader.py`의 리스크 상태를 `logs/risk_state_<mode>.json`에 저장한다.
...
- 텔레그램 `/daily` 명령 추가와 국내/해외 전략 파라미터 완전 분리
