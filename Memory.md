...
- 일일 손실/MDD 상태는 `logs/risk_state_<mode>.json`과 SQLite `equity_state` 테이블에 모드별로 저장한다.
...
- [x] `src/trader.py`: 해외 거래소별 잔고 병합, 해외 잔고 기반 매수 수량 계산, 국내/미국 전체청산, 보유 고점 전달, 시장별 전략, 스크리너/쿨다운/타임아웃/일일 리포트/equity_state 적용
- [x] `src/risk.py`: 트레일링 스탑 판단과 SQLite/JSON 리스크 상태 저장 추가
...
## 최신 검증 결과
- [x] `py -m unittest discover -s tests -p "test_*.py"`: 35 tests OK
