...
## 변경 파일
- [x] `autotrader.py`: KIS 호출 제한 통일, 토큰 캐시 삭제, 설정 재귀 병합, 스크리너, 종목별 타임아웃, 전체청산 문구 정리
...
- [x] `src/trader.py`: 해외 거래소별 잔고 병합, 해외 잔고 기반 매수 수량 계산, 국내/미국 전체청산, 보유 고점 전달, 스크리너/쿨다운/타임아웃 적용
...
- [x] `src/config.py`: `config.yaml` 누락/부분 설정에 대한 기본값 병합, `screener`/`process_timeout_sec` 설정 노출
- [x] `src/telegram_bot.py`: 상태 표시, 전체청산 문구, 봇 정지 대기 처리 정리
...
## 최신 검증 결과
- [x] `py -m unittest discover -s tests -p "test_*.py"`: 25 tests OK
...
- [ ] 텔레그램 `/daily` 명령과 국내/해외 전략 파라미터 완전 분리는 별도 범위로 검토한다.
