...
## 변경 파일
- [x] `autotrader.py`: KIS 호출 제한 통일, 토큰 캐시 삭제, 전체청산 문구 정리
- [x] `app.py`: 설정 저장/모드 전환 토큰 캐시 삭제, 전체청산 문구 정리
- [x] `src/kis_api.py`: 호출 제한 상수화, 최소 호출 간격 적용, 해외 잔고 거래소 메타데이터/예수금 필드 보강
- [x] `src/trader.py`: 해외 거래소별 잔고 병합, 해외 잔고 기반 매수 수량 계산, 국내/미국 전체청산, 보유 고점 전달
...
- [x] `src/config.py`: `config.yaml` 누락/부분 설정에 대한 기본값 병합, `screener` 설정 노출
- [x] `tests/test_portfolio_snapshot.py`: 호출 제한, 트레일링 스탑, 해외 잔고 병합, 국내/미국 전체청산, 매수가능금액, 리스크 상태 저장 테스트 추가
...
## 최신 검증 결과
- [x] `py -m unittest discover -s tests -p "test_*.py"`: 17 tests OK
...
## 주의 사항
- 이번 작업에서 실전 주문, 실제 청산, 계좌 자금 이동, 원격 push는 수행하지 않았다.
